...
public class order_backendorder_page2 extends BaseClass {
	...
	public order_backendorder_page2(WebDriver driver) {
		this.driver = driver;
	...
	public void setAdjustedPrice() throws Exception {
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", header_products_section);//this and next code written for making script=elmsBackEndOrderIP to run
	...
	private WebElement add_existing_registrant_email;
	...
	private WebElement select_existing_registrant;

	public void addExistingRegistrant() throws Exception {
		String existingregistrant_emailid_data = PropertyUtils.readProperty("existingRegistrantemailid");
		add_existing_registrant_email.sendKeys(existingregistrant_emailid_data);
		//new WebDriverWait(driver, 20).until(ExpectedConditions.elementToBeClickable(select_existing_registrant))
		//.click();

		new WebDriverWait(driver, Duration.ofSeconds(20)).until(ExpectedConditions.elementToBeClickable(select_existing_registrant))
		.click();
	...
	private WebElement existing_buyer_registrant_email;

	@FindBy(xpath = "//div[text()='Registrant - 3 of 3']//parent::div//child::input[contains(@name,'email_address')]//parent::div//descendant::li")
	private WebElement select_buyer_existing_registrant;

	public void addBuyerRegistrant() throws Exception {

		existing_buyer_registrant_email.sendKeys(customer_work_email);
		//new WebDriverWait(driver, 20).until(ExpectedConditions.elementToBeClickable(select_buyer_existing_registrant))
		//.click();//code to select from suggestion list
		new WebDriverWait(driver, Duration.ofSeconds(20)).until(ExpectedConditions.elementToBeClickable(select_buyer_existing_registrant))
		.click();//code to select from suggestion list
	...
	public void setPaylaterPaymentoption() throws Exception {
		...
		add_due_date.sendKeys(duedate);
		add_due_date.sendKeys(Keys.TAB);// this will click outside the due date field

		log.info("selected PAY LATER method and added current date as DUE DATE ");
	}
	...
	private WebElement link_clearall;
	...
	public void clearAllFilterLink() {
	...
	public void clickCreditMemoLink() throws Exception {
		try {
			if (link_credit_memo.isDisplayed()) {
				...
				// work
				{
					if ((msg_confirmation_credit_memo).isEnabled()) {
						log.info("offline refund confirmation msg is: " + msg_confirmation_credit_memo.getText());
						btn_confirmation_ok.click();
						log.info("offline refund confirmed");
					}
				} catch (Exception e) {
	...
	@FindBy(xpath = "//div[@id='modal-content-14']")
	...
	public void updateCustomerfnamelnameemail() throws Exception {
		...
		File screenshot2 = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
		try {
			FileUtils.copyFile(screenshot2, new File("target/screenshots/AccountInfoAftercustomerfnamelnameemailUPDATE.jpg"));
			log.info("ADDRESS INFO section screenshot captured to check updated info");
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
		...
		File screenshot = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
		try {
			FileUtils.copyFile(screenshot, new File("target/screenshots/AddressInfoAftercustomerfnamelnameemailUPDATE.jpg"));
			log.info("ADDRESS INFO section screenshot captured to check updated info");
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
		log.info("firstname and lastname displaying in Adress info section is: "+fnamelname_addressinfocustomername.getText());
	...
	public void updatemasteraccountwithuniquevalue() throws Exception {
		...
		if(masteraccount.getAttribute("value").equals(updatedmasteraccount))
	...
	@FindBy(xpath = "//div[@id='elmtmasteraccount']//li")
	...
	String updatedorderaccount;

	public void updateorderaccountwithuniquevalue() throws Exception {
		log.info("I am inside updateorderaccountwithuniquevalue method");
		Thread.sleep(2000);
